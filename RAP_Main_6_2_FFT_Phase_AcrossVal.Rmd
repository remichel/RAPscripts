...
# out files
MMpermEnd           = "_mm_acrossval_perm.txt"#"_mm_perm_acrossval.txt"
...
# subset data
data = phase_long[phase_long$freq == 4.8 & phase_long$var == "g",]
...
roseplot = ggplot(data, aes(x = deg)) +
    ...
    geom_segment(x = mean.circular(data$deg), xend = mean.circular(data$deg), 
               y = 0, yend = 2.8, 
               color = "grey40", size = 7, arrow = arrow(length = unit(1.5, "cm")))+  # arrow
    geom_histogram(binwidth = 360/nBins, 
                 color = "black", fill = "grey70", size = 1.2)+ # hist
...
if(do_save) ggsave(filename = paste0(plot_path,'main_mm_g_phase_roseplot_acrossval.png'), device = "png", height = 30, width = 30, units = "cm")
if(do_save) ggsave(filename = paste0(plot_path,'main_mm_g_phase_roseplot_acrossval.pdf'), device = "pdf", height = 30, width = 30, units = "cm")
```

...
testlist = list(freq = c(4.8, 9.6, 10.8),
     val = c("invalid", "invalid", "valid"),
     m = c("g", "sd", "sd"))
...
```{r phase plot, echo = F, out.width = '100%'}

# subset data
data = phase_long[phase_long$validity == "invalid" &
                    phase_long$freq == 9.6 &
                    phase_long$var == "sd",]

# compute phase in radians + degrees for plot
data$phase  = circular(data$phase, unit = "radians")
data$deg    = conversion.circular(data$phase, units = "degrees")

# prepare plot
nBins   = 30
breaks  = seq(-180, 170, by = 45)
labels  = c(seq(180, 350, by = 45), seq(0, 170, by = 45))


roseplot = ggplot(data, aes(x = deg)) +
    geom_vline(xintercept = breaks, color = "grey80", size = 1) + 
    geom_hline(yintercept = c(1,2,3), color = "grey80", size = 0.5)+
    geom_hline(yintercept = 3.2, color = "black", size = 1.5)+ # grid
  
    geom_segment(x = mean.circular(data$deg), xend = mean.circular(data$deg), 
               y = 0, yend = 2.8, 
               color = "sienna2", size = 7, arrow = arrow(length = unit(1.5, "cm")))+  # arrow
    geom_histogram(binwidth = 360/nBins, 
                 color = "black", fill = "sienna2", size = 1.2)+ # hist
  
    scale_x_continuous(breaks = breaks, labels = labels,expand = c(.002,0))+
    coord_polar(start = pi-pi/nBins)+
    ylab(NULL)+
    xlab(NULL)+
    theme_minimal()
...
roseplot + theme(axis.line = element_blank(),
        ...
        plot.margin = unit(c(.5,.5,.5,.5), unit = "cm"))

if(do_save) ggsave(filename = paste0(plot_path,'main_mm_sd_phase_roseplot.png'), device = "png", height = 30, width = 30, units = "cm")
if(do_save) ggsave(filename = paste0(plot_path,'main_mm_sd_phase_roseplot.pdf'), device = "pdf", height = 30, width = 30, units = "cm")
```
